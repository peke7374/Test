...
local INVISIBLE_OFFSET = Vector2.new(0, 4);
...
function EspObject:Construct()
	...
	self.drawings = {
		...
		visible = {
			...
			invisible = self:_create("Text", { Center = true, Visible = false }),
		},
		hidden = {
...
function EspObject:Update()
	...
	self.invisible = interface.getInvisible(self.player);
...
function EspObject:Render()
	...
	visible.invisible.Visible = enabled and onScreen and options.invisible;
	if visible.invisible.Visible then
		local invisible = visible.invisible;
		invisible.Text = self.invisible;
		invisible.Size = interface.sharedSettings.textSize;
		invisible.Font = interface.sharedSettings.textFont;
		invisible.Color = parseColor(self, options.invisibleColor[1]);
		invisible.Transparency = options.invisibleColor[2];
		invisible.Outline = options.invisibleOutline;
		invisible.OutlineColor = parseColor(self, options.invisibleOutlineColor, true);
		invisible.Position = (corners.topLeft + corners.topRight)*0.5 - Vector2.yAxis*invisible.TextBounds.Y - Vector2.new(0,15);
	end
	...
	if visible.tracer.Visible then
		...
		tracer.From =
			options.tracerOrigin == "Center" and viewportSize*0.5 or
			options.tracerOrigin == "Top" and viewportSize*Vector2.new(0.5, 0) or
			options.tracerOrigin == "Left" and viewportSize*Vector2.new(0, 0.5) or
			options.tracerOrigin == "Right" and viewportSize*Vector2.new(1, 0.5) or
			options.tracerOrigin == "Top Left" and viewportSize*Vector2.new(0, 0) or
			options.tracerOrigin == "Top Right" and viewportSize*Vector2.new(1, 0) or
			options.tracerOrigin == "Bottom Left" and viewportSize*Vector2.new(0, 1) or
			options.tracerOrigin == "Bottom Right" and viewportSize*Vector2.new(1, 1) or
			options.tracerOrigin == "Bottom" and viewportSize*Vector2.new(0.5, 1) or
			options.tracerOrigin == "Mouse" and Vector2.new(localPlayer:GetMouse().X, localPlayer:GetMouse().Y + game:GetService("GuiService"):GetGuiInset().Y);
...
local EspInterface = {
	...
	teamSettings = {
		enemy = {
			...
			invisible = false,
			invisibleColor = { Color3.new(1,1,1), 1 },
			invisibleOutline = true,
			invisibleOutlineColor = Color3.new(),
			distance = false,
...
function EspInterface.getInvisible(player)
    if player and player.Character and player.Character:FindFirstChild("head") and player.Character.head.Transparency >= 1 then
        return "[!] Admin [!]"
    end
...
repeat task.wait() until Characters:FindFirstChildWhichIsA("Model")
local RandomTarget = Characters:GetChildren()[math.random(1, #Characters:GetChildren())]
local CharacterList = {}

for i,v in pairs(getgc(true)) do 
    if (type(v) == "table" and rawget(v, RandomTarget.Name) and type(rawget(v, RandomTarget.Name)) == "string") then 
        CharacterList = v 
        break
    end
end

local CharPlayer = function(Player)
    for i,v in pairs(CharacterList) do
        if Player.Name ~= v then continue end
        local TargetCharacter = Characters:FindFirstChild(i)
...
for x,z in pairs(game.ReplicatedStorage.data.item:GetChildren()) do
	for a,b in pairs(z:GetChildren()) do
		table.insert(WeaponTable, b.Name)
	end
end

function EspInterface.getWeapon(player)
    if player and CharPlayer(player) then
		local char = CharPlayer(player)
        for i,v in ipairs(char:GetChildren()) do
            if v.Name == "prepared_weapon" and v:FindFirstChild("receiver") then
				for x,z in ipairs(v.receiver:GetChildren()) do
					if z:IsA("Attachment") and table.find(WeaponTable, z.Name) then
						return z.Name
					end
				end
			end
